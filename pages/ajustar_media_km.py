...
Agora calcula usando APENAS as 3 últimas visitas válidas (compatível com utils.py)
"""
...
def app():
    ...
    if delta_dias > 0 and delta_km >= 0:
        ...
        # Mostrar informações
        st.info(f"📊 Calculando com base em {len(ultimas_3)} visitas (últimas visitas)")
        ...
        if st.button("💾 Salvar Média e Corrigir Histórico", type="primary", use_container_width=True):
            try:
                with conn.cursor() as cursor:
                    # 1. Atualiza TODAS as visitas (não apenas as 3)
