...
# =========================
# Utilitários de imagem (Sua versão original)
# =========================
...
# -------- Helpers para exportação PDF (ATUALIZADO PARA LAUDO COMPLETO) --------
...
def _render_report_image(laudo: dict, meta: dict, obs: str, collage: Image.Image) -> Image.Image:
    ...
    # Compatibilidade: checa se é o laudo novo ou antigo
    ...
    y = draw_wrapped_text("Laudo Técnico de Análise de Pneus", title_font, y) + H_PAD
...
for eixo in laudo.get("eixos", []):
with st.container(border=True):
st.markdown(f"### {eixo.get('titulo', 'Eixo')}")
st.write(eixo.get("diagnostico_global", "N/A"))
# O resto da sua renderização original viria aqui...
