...
"""Fallback: analisa UMA colagem. Mantido para estabilidade, mas não será usado na lógica principal."""
# Esta função pode ser simplificada ou removida se a chamada principal for estável
return {"erro": "Fallback por eixo desativado na nova versão."}

=========================
UI helpers (SEÇÃO ATUALIZADA)
=========================
def _render_laudo_ui(laudo: dict, meta: dict, obs: str):
"""ATUALIZADO - Renderiza o novo laudo profissional na tela do Streamlit."""
...
=========================
UI (SEU CÓDIGO ORIGINAL COM PEQUENOS AJUSTES)
=========================
def app():
...
if "laudo" in st.session_state:
    # A lógica de renderização agora usa a nova função
    ...
    with col_exp1:
         if "ultima_colagem" in st.session_state and st.session_state.get("ultima_colagem") is not None:
            # Lógica de PDF permanece, mas pode ser melhorada no futuro
    ...
    msg = (
        f"Olá! Segue resumo da análise de pneus para o veículo {st.session_state.get('meta',{}).get('placa', '')}:\n\n"
        f"{resumo_wpp}"
    )
