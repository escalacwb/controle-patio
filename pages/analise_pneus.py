...
def _wrap_text(draw: ImageDraw.ImageDraw, text: str, font, max_w: int) -> List[str]:
    ...
    for paragraph in (text or "").split("\n"):
        ...
        for w in words:
            test = (cur + " " + w).strip()
            # Fallback for Pillow versions without textbbox
...
def _render_report_image(laudo: dict, meta: dict, collage: Image.Image) -> Image.Image:
    ...
    height += get_text_height("2. Diagnóstico Global", h2_font, W) + H_PAD
    height += get_text_height("\n".join([f"• {item}" for item in dg.get('problemas_sistemicos',[])]), body_font, W, 20) + H_PAD
    height += get_text_height("3. Plano de Ação Crítico", h2_font, W) + H_PAD
    ...
    y = draw_wrapped_text("3. Plano de Ação Crítico", h2_font, y) + H_PAD
    y = draw_wrapped_text("\n".join([f"• {item}" for item in plano.get('critico_risco_imediato',[])]), body_font, y, indent=20, color=(200,0,0)) + H_PAD
