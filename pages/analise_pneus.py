...
def _grid_2x2_labeled(lt, lb, rt, rb, labels):
    if not all([lt, lb, rt, rb]): return Image.new("RGB", (100, 100), "white")
    left_w = min(lt.width, lb.width)
    right_w = min(rt.width, rb.width)
    lt = lt.resize((left_w, int(lt.height * (left_w / lt.width))), Image.LANCZOS)
    lb = lb.resize((left_w, int(lb.height * (left_w / lb.width))), Image.LANCZOS)
    rt = rt.resize((right_w, int(rt.height * (right_w / rt.width))), Image.LANCZOS)
    rb = rb.resize((right_w, int(rb.height * (right_w / rb.width))), Image.LANCZOS)
    top_h = max(lt.height, rt.height)
    bot_h = max(lb.height, rb.height)
    lt_padded, rt_padded = Image.new("RGB", (lt.width, top_h), "white"), Image.new("RGB", (rt.width, top_h), "white")
    lb_padded, rb_padded = Image.new("RGB", (lb.width, bot_h), "white"), Image.new("RGB", (rb.width, bot_h), "white")
    lt_padded.paste(lt, (0,0))
    rt_padded.paste(rt, (0,0))
    lb_padded.paste(lb, (0,0))
    rb_padded.paste(rb, (0,0))
    total_w, total_h = left_w + right_w, top_h + bot_h
    out = Image.new("RGB", (total_w, total_h), "white")
    out.paste(lt_padded, (0, 0))
    out.paste(rt_padded, (left_w, 0))
    out.paste(lb_padded, (0, top_h))
    out.paste(rb_padded, (left_w, top_h))
    
    if labels.get("title"): _draw_label(out, labels["title"], xy=(8, 8))
...
def _stack_vertical_center(collages: List[Image.Image], titles: List[str]) -> Image.Image:
    if not collages: return Image.new("RGB", (800, 600), "white")
...
# ATUALIZADO: Utilitários de PDF e Relatório
# =========================
...
def _wrap_text(draw: ImageDraw.ImageDraw, text: str, font, max_w: int) -> List[str]:
    ...
    for paragraph in (text or "").split("\n"):
        ...
        for w in words:
            test = (cur + " " + w).strip()
            bbox = draw.textbbox((0,0), test, font=font) if hasattr(draw, 'textbbox') else (0,0, len(test)*font.size*0.6, font.size)
...
def _render_report_image(laudo: dict, meta: dict, collage: Image.Image) -> Image.Image:
    ...
    title_font, h2_font, h3_font, body_font, caption_font = _get_font(32), _get_font(26), _get_font(22), _get_font(18), _get_font(16)
    
    dummy_draw = ImageDraw.Draw(Image.new("RGB", (W, 10), "white"))
    
    def get_text_height(text, font, max_w, indent=0):
        if not text or not isinstance(text, str): return 0
        return len(_wrap_text(dummy_draw, text, font, max_w - indent)) * (font.size + 5)

    height = P
    # Calcular altura total necessária
    height += get_text_height(f"Laudo Técnico de Análise Visual de Pneus - Placa: {meta.get('placa', 'N/A')}", title_font, W) + H_PAD
    height += get_text_height(f"Data: {datetime.now().strftime('%d/%m/%Y %H:%M')}", body_font, W) + H_PAD * 2

    height += get_text_height("1. Resumo Executivo", h2_font, W) + H_PAD
    height += get_text_height(laudo.get('resumo_executivo', 'N/A'), body_font, W) + H_PAD * 2
    
    height += get_text_height("2. Diagnóstico Global do Veículo", h2_font, W) + H_PAD
    dg = laudo.get('diagnostico_global_veiculo', {})
    height += get_text_height("Problemas Sistêmicos:", h3_font, W)
    height += get_text_height("\n".join([f"• {item}" for item in dg.get('problemas_sistemicos',[])]), body_font, W, 20) + H_PAD
    height += get_text_height("Componentes para Inspeção Prioritária:", h3_font, W)
    height += get_text_height("\n".join([f"• {item}" for item in dg.get('componentes_mecanicos_suspeitos',[])]), body_font, W, 20) + H_PAD * 2
    
    height += get_text_height("3. Análise Detalhada por Eixo", h2_font, W) + H_PAD
    for eixo in laudo.get('analise_detalhada_eixos', []):
        height += get_text_height(eixo.get('titulo_eixo', 'Eixo'), h3_font, W) + H_PAD
        for pneu in eixo.get('analise_pneus', []):
            for defeito in pneu.get('defeitos', []):
                height += get_text_height(f"{defeito.get('nome_defeito')} ({defeito.get('urgencia')})", body_font, W, 20)
                exp = defeito.get('explicacao', {})
                height += get_text_height(f"Risco: {exp.get('risco_nao_corrigir')}", caption_font, W, 40) + H_PAD
    
    height += get_text_height("4. Plano de Ação", h2_font, W) + H_PAD
    plano = laudo.get('plano_de_acao', {})
    height += get_text_height("Ações Críticas:", h3_font, W)
    height += get_text_height("\n".join([f"• {item}" for item in plano.get('critico_risco_imediato',[])]), body_font, W, 20) + H_PAD
    ...
    y = draw_wrapped_text(f"Laudo Técnico de Análise Visual de Pneus - Placa: {meta.get('placa', 'N/A')}", title_font, y) + H_PAD
    ...
    y = draw_wrapped_text("\n".join([f"• {item}" for item in dg.get('componentes_mecanicos_suspeitos',[])]), body_font, y, indent=20) + H_PAD * 2
    
    y = draw_wrapped_text("3. Análise Detalhada por Eixo", h2_font, y) + H_PAD
    # ... (a renderização completa do eixo é complexa e omitida aqui para manter a legibilidade, mas seguiria a lógica de cálculo de altura)
...
# ATUALIZADO: OpenAI / Prompt helpers
# =========================
def _build_multimodal_message(data_url: str, meta: dict, obs: str, axis_titles: List[str]) -> list:
    """Constrói o prompt de usuário com base no novo padrão exigido pelo gestor."""
    prompt_usuario = f"""
...
ATUALIZADO: OpenAI callers
=========================
def _call_openai(model_name: str, messages: list) -> dict:
...
try:
    ...
    # Tenta extrair JSON de um texto mal formatado
    try:
        start = raw_text.find('{')
...
if laudo.get('tabela_visao_geral'):
    df_data = []
    for item in laudo['tabela_visao_geral']:
        urgencia = item.get('urgencia', 'N/A')
        df_data.append({
            "Posição": item.get('posicao'),
            "Principal Defeito": item.get('principal_defeito'),
            "Urgência": urgency_map.get(urgencia, urgencia)
        })
...
with col1:
    ...
    if st.button("🚀 Gerar Laudo Profissional", type="primary", use_container_width=True, disabled=not st.session_state.axes):
        ...
        messages=[
                ...
                {"role": "user", "content": _build_multimodal_message(data_url, meta, observacao, titles)},
            ]

        with st.spinner("IA analisando... Isso pode levar até 2 minutos."):
            laudo = _call_openai(modelo, messages)
...
with col2:
    if "laudo" in st.session_state:
        ...
        with st.sidebar:
            ...
            resumo_wpp = st.session_state["laudo"].get("whatsapp_resumo", "Análise de pneus concluída.")
