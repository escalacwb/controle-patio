...
A imagem fornecida é uma montagem vertical de colagens **2x3** (2 colunas x 3 linhas) por eixo.
...
**Extração de Informações da LATERAL:**
Quando disponível, **extraia o máximo de informações da lateral (sidewall)**: **marca, modelo, medida (ex.: 295/80R22.5), índice de carga/velocidade, construção (radial/diagonal), DOT completo (semana/ano e planta), marcações de fogo (incêndio) ou de recapagem, sentido de rotação, TWI, tipo de aplicação (on/off, regional/urbano/longa distância)**.
...
**C. Análise Detalhada por Eixo:** Para cada eixo:
  ...
  - **Análise por Pneu (Motorista e Oposto):** Para cada pneu:
    - **Defeitos:** Para CADA defeito encontrado:
      ...
      - **`localizacao_visual`**: **Descreva textualmente onde olhar na foto** (ex: "Ombro externo do pneu", "Blocos centrais da banda de rodagem", **"lateral/sidewall acima do DOT"**).
    ...
    - **`identificacao_sidewall`** (quando visível): campos com **marca, modelo, medida, índice carga/velocidade, construção, DOT, marcações de fogo ou recapagem, sentido, TWI, aplicação**.
